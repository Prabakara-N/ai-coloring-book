...
export const BACK_COVER_PROMPT_TEMPLATE = (opts: {
  ...
  return [
    ...
    "🎨 TWO-LAYER BACKGROUND — MATCH THE FRONT COVER (CRITICAL): A style reference image of the front cover is provided. Pick the dominant accent color from the front. The back cover is split into TWO HORIZONTAL LAYERS that share the same hue family but differ in brightness: TOP LAYER — a VERY THIN header band at the very top of the cover, EXACTLY 4-6% of the total cover height (this is a SLIM RIBBON / DECORATIVE STRIP, like a sliver at the top — NOT a chunky block, NOT a quarter-page header. Visualize: if the cover is 11 inches tall, this band is 0.4-0.6 inches — barely a finger-width). Use a slightly DARKER, more saturated shade of the chosen color. BOTTOM LAYER — the remaining 94-96% of the cover (almost the entire surface, where text + barcode + description go). Use a noticeably LIGHTER, softer pastel version of the same color. The transition between layers is a CLEAN STRAIGHT HORIZONTAL EDGE (no gradient, no fade — like two solid color bands meeting). Both layers carry a VERY SUBTLE paper-texture speckle (recycled cardstock feel). Examples: front sky-blue → top deeper navy/blue, body pale cream-blue. Front mint green → top deeper forest/sage green, body pale mint. Front coral → top warm terracotta, body pale peach. NO patterns, NO scenes, NO decorations on either layer.",
