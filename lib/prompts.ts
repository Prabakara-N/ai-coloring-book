...
export const BACK_COVER_PROMPT_TEMPLATE = (opts: {
  ...
  return [
    ...
    "🎯 OVERALL AESTHETIC — CLEAN, MINIMAL, ELEGANT: Think a professional published book back, not a cartoon sticker. Inspired by Penguin Classics / Tuttle / modern indie picture-book backs. NO illustration, NO characters, NO scene, NO objects, NO drawn cartoon elements. Just a soft textured background + one elegant tagline floating freely + an EMPTY white rectangle bottom-right (Amazon prints the barcode on top of it). Lots of breathing room.",
    "🎨 TWO-LAYER BACKGROUND — MATCH THE FRONT COVER (CRITICAL): A style reference image of the front cover is provided. Pick the dominant accent color from the front. The back cover is split into TWO HORIZONTAL LAYERS that share the same hue family but differ in brightness: TOP LAYER (top ~18% of the cover, like a header band): a slightly DARKER, more saturated shade of the chosen color. BOTTOM LAYER (remaining ~82%, the main body where text + barcode go): a noticeably LIGHTER, softer pastel version of the same color. The transition between layers is a CLEAN STRAIGHT HORIZONTAL EDGE (no gradient, no fade — like two solid color bands meeting). Both layers carry a VERY SUBTLE paper-texture speckle (recycled cardstock feel). Examples: front sky-blue → top deeper navy/blue, body pale cream-blue. Front mint green → top deeper forest/sage green, body pale mint. Front coral → top warm terracotta, body pale peach. NO patterns, NO scenes, NO decorations on either layer.",
    ...
    "📐 BARCODE SAFE-ZONE — EMPTY WHITE RECTANGLE (STRICT GEOMETRY): In the bottom-right corner draw a SHARP CRISP RECTANGULAR shape — exactly 28% of the cover width by 18% of the cover height — sitting precisely 4% from the right edge and 4% from the bottom edge. CRITICAL GEOMETRY: ALL FOUR CORNERS are perfectly 90-degree right angles, NOT rounded, NOT softened, NOT fuzzy. The rectangle's four sides are perfectly straight, NOT wavy, NOT curved. The interior is filled with PURE 100% WHITE (#FFFFFF) — no off-white, no shading. There is a THIN 1-PIXEL HAIRLINE GREY (#999) border outline around the rectangle so its sharp edges are clearly visible against the colored background behind. INSIDE the rectangle: ABSOLUTELY NOTHING — no barcode lines, no ISBN digits, no numbers, no text, no patterns, no marks of any kind. It must be 100% empty white space. Amazon prints the actual ISBN barcode on top of this area at production time, so we must NOT pre-print one (it would conflict and be rejected).",
    ...
    opts.ageLabel ?? "",
    "Crisp printable quality at 300 DPI. No watermark, no URL, no author name, no extra text beyond the elegant tagline (the bottom-right rectangle stays empty white).",
