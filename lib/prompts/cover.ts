...
export const COLOR_COVER_PROMPT_TEMPLATE = (opts: {
  ...
  return [
    ...
    `3) SIDE PLAQUE — a small plaque / sign / banner shape (about 18-24% of cover width) styled per the overlay design language above, tilted ~5-10 degrees, with three short stacked lines of friendly capitals (the first line in an accent color from the design language, the next two in the dominant readable color): "${plaqueLines[0]}" / "${plaqueLines[1]}" / "${plaqueLines[2]}". PLACEMENT — STRICT (this is the most common cover-design failure): the plaque MUST sit in a CLEAR EMPTY ZONE of the cover where NO character body, NO character face, and NO important prop is drawn. NEVER cover a character — not their face, not their body, not their feet, not their tail, not their accessory. Scan the cover's left half top-to-bottom for empty background area (the parts of the scene with no character or prop on them) and place the plaque there. Best zones in priority order: (a) bottom-left corner, resting on whatever surface naturally exists at the bottom of THIS scene; (b) top-left, on the empty background near the title; (c) a vertical strip of empty background between the cover's left edge and the leftmost character. If NONE of those zones have enough empty room, SHRINK the plaque to 14-18% width OR move it to the right side — anywhere clear. INTEGRATION — the plaque looks like a physical object that BELONGS in this book's world; the object's material, shape, and how it attaches to the surrounding scene must be DERIVED from this book's actual setting, NEVER defaulted to wood/fence/branch/chalkboard unless those genuinely fit. It must look like part of the scene that belongs there, not a UI overlay.`,
