...
interface CoverPairProps {
   ...
   * badge, side plaque, and bottom strip on the front cover. Empty string
   * means "use the default clean overlays". Edits take effect on the next
   * front-cover regenerate.
   */
    ...
    <div className="rounded-3xl p-5 md:p-6 bg-zinc-900/60 backdrop-blur-xl border border-white/10 space-y-5">
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <BadgeStyleField
            value={coverBadgeStyle}
            onChange={onCoverBadgeStyleChange}
          />
    ...
    <div className="max-w-md mx-auto">
      <div className="flex items-baseline justify-between mb-1.5">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-neutral-400">
          Overlay design language
        </p>
        <span
          className={cn(
            "text-[9px] font-mono",
            overLimit ? "text-rose-400" : "text-neutral-500",
          )}
        >
          {charCount}/200
        </span>
      ...
      <textarea
        ...
        rows={2}
        maxLength={240}
        placeholder="Auto-generated by the planner. Describes the visual style of the page-count badge, side plaque, and bottom strip so they match the book's world. Empty = clean default."
        className="w-full text-xs leading-relaxed rounded-xl bg-black/40 border border-white/10 px-3 py-2 text-neutral-200 placeholder:text-neutral-500 focus:outline-hidden focus:border-violet-400/60 focus:ring-1 focus:ring-violet-400/30 resize-none"
      />
      <p className="text-[10px] text-neutral-500 mt-1 leading-snug">
        Edits take effect on the next front-cover regenerate. Keep it to one
        sentence describing materials, shapes, and color motifs.
      </p>
    </div>
  );
}
